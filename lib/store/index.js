    ...
    put(index, buf, cb) {
        ...
        if (self.length === Infinity) {
            ...
            var tasks = targets.map((target) => {
                return function (cb) {
                    if (!self.selectedFiles.has(target.file.path)) {
                        return target.file.cache(target.offset, buf.slice(target.from, target.to)).then(cb).catch(cb)
                    }
                    target.file.write(target.offset, buf.slice(target.from, target.to)).then(cb).catch(cb)
                }
            })
            parallel(tasks, cb)
        }
